...
+ (BOOL)needsDisplayForKey:(NSString *)key
{
    if ([key isEqualToString:@"progress"]) {
        return YES;
    } else if ([key isEqualToString:@"startProgress"]) {
        return YES;
    } else {
...
- (void)drawInContext:(CGContextRef)context
{
    ...
    CGFloat startProgress = MIN(self.startProgress, 1.0f - FLT_EPSILON);
    CGFloat radians = 0;
    CGFloat startRadians = 0;
    
    if (clockwise)
    {
        startRadians = (float)((startProgress * 2.0f * M_PI) - M_PI_2);
        radians = (float)((progress * 2.0f * M_PI) + startRadians);
    }
    else
    {
        startRadians = (float)(3 * M_PI_2 - (startProgress * 2.0f * M_PI));
        radians = (float)(startRadians - (progress * 2.0f * M_PI));
    }
    if ([self.trackTintColor isEqual:[UIColor clearColor]]){
...
{
    return self.circularProgressLayer.startProgress;
}

- (void)setStartProgress:(CGFloat)startProgress
{
    CGFloat pinnedProgress = startProgress;
    if (pinnedProgress < 0){
        self.clockwiseProgress = YES;
        pinnedProgress = - pinnedProgress;
    }
    if (pinnedProgress > 1){
        pinnedProgress = pinnedProgress - (int)(pinnedProgress);
    }
    self.circularProgressLayer.startProgress = pinnedProgress;
//    [self.circularProgressLayer setNeedsDisplay];
}
...
//    CGFloat pinnedProgress = MIN(MAX(progress, 0.0f), 1.0f);
    CGFloat pinnedProgress = progress;
    if (pinnedProgress < 0){
        self.clockwiseProgress = YES;
        pinnedProgress = - pinnedProgress;
    }
...
- (void)icd_addAnimation:(CAAnimation *)animation forKey:(NSString *)key{
    [self.circularProgressLayer addAnimation:animation forKey:key];
}
